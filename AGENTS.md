...
- Sponsor/partner logos live at the root of `public/` as `<org>-logo.<ext>`, all wired through `lib/sponsors/partners.ts`: `aulden-logo.svg`, `schwans-logo.png`, `united-way-logo.png`, `mnsp-logo.png`, `visit-marshall-logo.png`, `kwik-trip-logo.svg`, `hyvee-logo.svg`, `smsu-math-cs-club-logo.png`, `rise-research-logo.png`. `aulden-logo-dark.svg` is the one exception: white-ink variant hard-coded in `components/footer.tsx` for the dark footer.
