...
- **The waitlist outlives the Sept 8 registration deadline on purpose.** `WAITLIST_CLOSE_AT` is *aliased* to `EVENT_START_AT` (not copied), so sign-ups run right up to doors, 8 AM Sept 12. Two intended consequences of that equality: the `"closed"` phase is unreachable (`getEventPhase` tests `"live"` first), and the countdown never shows "Event begins in". Both return automatically if the waitlist is ever given its own earlier literal. `__tests__/event-phase.test.ts` asserts a Sept 10 clock still reports `"open"` — do not "fix" that test by moving the deadline back.
- `components/waitlist-cta.tsx` - the ONLY way to render a sign-up CTA. `WaitlistCta` variants: header-desktop, header-mobile, hero, section, footer-link; the open-phase label is always "Join the waitlist". Switches to "Waitlist closed" when doors open (8 AM Sept 12) and to Devpost-gallery CTAs after the event. Every call site passes `initialPhase` so SSR HTML and hydration agree. The same file exports `WaitlistNote` (the `WAITLIST_NOTE` sentence) and `WaitlistDeadline`; both are *siblings*, not part of the CTA, so each surface places them in its own layout. Both **self-gate on the phase** as well as taking the caller's `phase === "open"` check — pages are ISR'd hourly, and that second gate is what stops a stale cache showing the note under a "Waitlist closed" button. Use `colorClassName` to replace their colour preset outright rather than fighting Tailwind precedence.
